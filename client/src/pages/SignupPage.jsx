...
export default function SignupPage() {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      setAuth(
        { firstName: data.firstName, lastName: data.lastName, email: data.email, organizationName: data.organization },
        'mock-jwt-token',
      );
      toast.success('Account created! Welcome to Columbus.');
      navigate('/dashboard');
    } catch {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-md py-xl">
      ...
      <main className="w-full max-w-[480px] flex flex-col gap-xl">
        <div className="flex flex-col items-center text-center gap-sm">
          ...
          <p className="text-body-lg text-on-surface-variant">Get your team onboarded in minutes.</p>
        </div>
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-xl shadow-card">
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-lg">
            <div className="grid grid-cols-2 gap-md">
              <Input label="First name" placeholder="Sarah" error={errors.firstName?.message} {...register('firstName')} />
              <Input label="Last name"  placeholder="Chen"  error={errors.lastName?.message}  {...register('lastName')} />
            </div>

            <Input label="Work email" type="email" placeholder="sarah@company.com" error={errors.email?.message} {...register('email')} />
            <Input label="Organization" placeholder="Acme Corp" error={errors.organization?.message} {...register('organization')} />
